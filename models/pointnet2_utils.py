...
class PointNetSetAbstractionMsg(nn.Module):
    def __init__(self, npoint, radius_list, nsample_list, in_channel, mlp_list):
        """
        in_channel: feature channel dimension of 'points' input
        xyz will be concatenated internally (+3)
        """
        ...
        for i in range(len(mlp_list)):
            convs = nn.ModuleList()
            bns = nn.ModuleList()
            last_channel = in_channel + 3  # concatenate grouped_xyz_norm
    ...
    def forward(self, xyz, points):
        ...
        for i, radius in enumerate(self.radius_list):
            ...
            grouped_points = grouped_points.permute(0, 3, 1, 2)        # [B, D+3, S, K]
